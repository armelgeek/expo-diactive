...
-- Create point_shares table with correct references
...
CREATE POLICY "Users can view their point shares"
  ...
  FOR SELECT
  USING (
    sender_id IN (SELECT id FROM profiles WHERE auth.uid() = auth.uid()) OR 
    receiver_id IN (SELECT id FROM profiles WHERE auth.uid() = auth.uid())
  );
...
CREATE POLICY "Users can create point shares"
  ...
  FOR INSERT
  WITH CHECK (
    sender_id IN (SELECT id FROM profiles WHERE auth.uid() = auth.uid())
  );
...
BEGIN
  IF NEW.status = 'accepted' AND OLD.status = 'pending' THEN
    ...
    VALUES (
      (SELECT auth.uid() FROM profiles WHERE id = OLD.sender_id),
      CURRENT_DATE,
      -OLD.points
    )
    ...
    INSERT INTO daily_steps (user_id, date, points_earned)
    VALUES (
      (SELECT auth.uid() FROM profiles WHERE id = OLD.receiver_id),
      CURRENT_DATE,
      OLD.points
    )
    ON CONFLICT (user_id, date) DO UPDATE
